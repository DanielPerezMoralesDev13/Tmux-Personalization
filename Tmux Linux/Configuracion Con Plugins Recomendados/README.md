...
- *Este comentario inicial indica que el archivo no debe ser editado directamente. En su lugar, se recomienda crear un archivo de personalización `.local` para hacer cambios. El archivo está bajo las licencias WTFPL y MIT, y fue creado por Gregory Pakosz.*
...
- **setw -g xterm-keys on:** *Habilita el soporte para las teclas de función de `xterm`.*
- **set -s escape-time 10:** *Reduce el tiempo de espera para las secuencias de comandos de escape.*
- **set -sg repeat-time 600:** *Aumenta el tiempo de repetición para las combinaciones de teclas.*
- **set -s focus-events on:** *Habilita eventos de enfoque en las terminales.*
- **set -g prefix2 C-a:** *Configura una segunda combinación de teclas prefijo (C-a) compatible con GNU Screen.*
- **bind C-a send-prefix -2:** *Envia el prefijo secundario (C-a) al servidor de tmux.*

### ***Edición y Recarga de Configuración***
...
- **bind e:** *Crea una nueva ventana para editar el archivo de configuración `.local`.*
- **bind r:** *Recarga la configuración actual de **tmux** y muestra un mensaje de confirmación.*
...
- **set -g base-index 1:** *Establece que la numeración de ventanas comience en 1.*
- **setw -g pane-base-index 1:** *Establece que la numeración de paneles comience en 1.*
- **setw -g automatic-rename on:** *Renombra automáticamente la ventana para reflejar el programa actual.*
- **set -g renumber-windows on:** *Reorganiza la numeración de las ventanas cuando se cierra una ventana.*
- **set -g set-titles on:** *Establece el título de la terminal.*
- **set -g display-panes-time 800:** *Establece el tiempo de visualización de los indicadores de paneles.*
- **set -g display-time 1000:** *Establece el tiempo de visualización de mensajes de estado.*

### ***Navegación y Creación de Sesiones***
...
- **bind C-c:** *Crea una nueva sesión.*
- **bind C-f:** *Busca una sesión y cambia a ella.*
- **bind BTab:** *Cambia al último cliente activo.*
- **bind -** *y* **bind _:** *Divide la ventana en paneles horizontal o verticalmente.*
- **bind -r h/j/k/l:** *Navega entre los paneles (izquierda, abajo, arriba, derecha).*
- **bind >/<:** *Intercambia el panel actual con el siguiente o anterior.*
- **bind +:** *Maximiza el panel actual.*
- **bind -r H/J/K/L:** *Redimensiona los paneles.*
- **bind -r C-h/C-l:** *Navega entre ventanas.*
- **bind Tab:** *Cambia a la última ventana activa.*

### ***Modo de Copia y Portapapeles***
...
- **bind Enter:** *Entra en modo de copia.*
- **bind -T copy-mode-vi v/C-v:** *Inicia la selección de texto en modo visual.*
- **bind -T copy-mode-vi y:** *Copia la selección al portapapeles y sale del modo de copia.*
- **bind -T copy-mode-vi H/L:** *Mueve el cursor al inicio o final de la línea.*
- **Comandos de copia al portapapeles:** *Dependiendo del entorno (X11, Wayland, macOS, Windows), se configura cómo copiar el texto seleccionado al portapapeles del sistema.*

### ***Configuraciones de Buffers***
...
- **bind b:** *Lista los buffers disponibles.*
- **bind p:** *Pega desde el buffer superior.*
- **bind P:** *Permite elegir qué buffer pegar.*

### ***Configuraciones Avanzadas***

```bash
%if #{==:#{TMUX_PROGRAM},}
  run 'TMUX_PROGRAM="$(command -v tmux || printf tmux)"; "$TMUX_PROGRAM" -S #{socket_path} set-environment -g TMUX_PROGRAM "$TMUX_PROGRAM"'
%endif
%if #{==:#{TMUX_SOCKET},}
...
%if #{==:#{TMUX_CONF_LOCAL
  ...
  run '"$TMUX_PROGRAM" set-environment -g TMUX_CONF_LOCAL $(for conf in "$HOME/.tmux.conf.local" "$XDG_CONFIG_HOME/tmux/tmux.conf.local" "$HOME/.config/tmux/tmux.conf.local"; do [ -f "$conf" ] && printf "%s" "$conf" && break; done)'
%endif
```

- **TMUX_PROGRAM/TMUX_SOCKET/TMUX_CONF:** *Estos comandos detectan y configuran las variables de entorno necesarias para el correcto funcionamiento de **tmux**, dependiendo de la configuración y las rutas de archivos.*

### ***Comandos para Maximizar Paneles y Compatibilidad***

```bash
maximize_pane_fullscreen='resizep -Z; set -g status off'
maximize_pane_fullscreen_restore='resizep -Z; set -g status on'

maximize_pane='if [ #{pane_width} -eq #{window_width} -a #{pane_height} -eq #{window_height} ]; then
  if [ #{window_zoomed_flag} -ne 1 ]; then
    set -g status off \;\
    display "[maximized]" \;\
    resize-pane -Z \;\
    set -g @maximized true
  else
    set -g status on \;\
    resize-pane -Z \;\
    set -g @maximized false
  fi
else
...
maximize_restore_pane='if [ #{window_zoomed_flag} -ne 1 ]; then
  if [ -z "#{@maximized}" ]; then
    resize-pane -Z \;\
    display "[restored]" \;\
    set -g @maximized true
...
- **maximize_pane** *y* **maximize_restore_pane:** *Estos scripts permiten maximizar un panel a pantalla completa o restaurarlo a su tamaño original.*

*Esta configuración es bastante completa y está diseñada para mejorar la experiencia de usuario en **tmux**, con una serie de atajos y configuraciones que simplifican el trabajo con múltiples ventanas y paneles.*
